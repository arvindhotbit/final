...
export class AdminReportComponent implements OnInit {
  ...
  filterCol(name)
  {
   return this.column_show_hide.includes(name)
  }
 ...
 {
   this.column_show_hide.push(obj);
 } 
else
{
  var x = this.column_show_hide.indexOf(obj);
  this.column_show_hide.splice(x,1);
}
 console.log(this.column_show_hide);
  }

  exportAsXLSX():void {
    this._tableservice.exportAsExcelFile(this.showdata, 'sample');
  }
  ...
  onZoneChange(zonevalue: any) {
    ...
    var obj = {
      ...
      "USER_ID": this.UserId,
      "TABLE_NAME": this.pageshistory
    };

    this._tableservice.getchangezonereportlist(obj).subscribe((res) => {
      this.showdata = res.result;
  ...
  onpageChange(pageshistory: any) {
    ...
    this._tableservice.getchangepagehistroylist(obj).subscribe((res) => {
      this.showdata = res.result;
      // this.toastr.success(res.message, 'zonelist');
  ...
  unseen() {
    this._tableservice.adminreportlist().subscribe((res) => {
      this.showdata = res.result;
