...
export class SensitiveWordComponent implements OnInit {
  ...
  updateform = () => {
    if (this._tableservice.selectsensitive.REF_KEY == "") {
  ...
  postChangetype(change_type) {
    if (this.zonevalue == null) {
      ...
      this._tableservice.sensitive_Change_Type(zonetype).subscribe((res) => {
        this.showdatapart = res.result;
  ...
  submitform(form: NgForm) {
    ...
    this._tableservice.postsensitive(form.value).subscribe((res) => {
      this.toastr.success(res.message, 'Sensitive Words');

    }, (error) => {                              //Error callback
      console.error('error caught in component')
      this.toastr.error(error, 'Sensitive Words');
  ...
  deleteSelected(form: NgForm) {
    if (this.masterSelected.length <= 0) {
      ...
      this._tableservice.deletesensitive(this.masterSelected.join(',')).subscribe((res) => {
